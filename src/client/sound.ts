...
export const subscribeSoundSettings = (fn: (s: SoundSettings) => void): (() => void) => {
  listeners.add(fn);
  return () => listeners.delete(fn);
};

export const updateSoundSettings = (patch: Partial<SoundSettings>): void => {
  settings = { ...settings, ...patch };
  persist();
  if ('musicEnabled' in patch) {
    if (settings.musicEnabled) startAmbient();
    else stopAmbient();
  }
  if ('musicVolume' in patch) setAmbientVolume(settings.musicVolume);
  notify();
};
...
const tone = (
  freq: number,
  duration: number,
  opts: { type?: OscillatorType; delay?: number; volume?: number } = {}
) => {
  const audio = ensureContext();
  if (!audio || !settings.sfxEnabled) return;
...
const synthSfx = {
  ...
  levelUp: () =>
    [392, 523.25, 659.25, 783.99, 1046.5].forEach((f, i) => tone(f, 0.22, { delay: i * 0.09, volume: 0.4 })),
...
const missingFiles = new Set<SfxKey>();
const sfxElements = new Map<SfxKey, HTMLAudioElement>();

const playSfxFile = (key: SfxKey): void => {
...
export const sfx: Record<SfxKey, () => void> = Object.fromEntries(
  (Object.keys(synthSfx) as SfxKey[]).map((key) => [
    key,
    () => {
...
type SynthAmbientNodes = { master: GainNode; stop: () => void };
let synthAmbient: SynthAmbientNodes | null = null;

const startSynthAmbient = (): void => {
  const audio = ensureContext();
  if (!audio || synthAmbient) return;
...
const setAmbientVolume = (volume: number): void => {
  if (bgmElement) bgmElement.volume = volume;
  if (synthAmbient) synthAmbient.master.gain.value = volume * 0.2;
};
