...
const toPublicPost = (
  ...
  category: cipher.category as CipherPost['category'],
  // Defensive fallback for cipher records created before language was tracked.
  language: (cipher.language ?? 'English') as CipherPost['language'],
...
const computeProgressInfo = (profile: { xp: number; currentStreak: number; lastActiveDate: string }) => {
...
api.post('/submit-cipher', async (c) => {
  ...
  const result = await createCipherPost(
    body.emojis,
    body.answer,
    Boolean(body.hardMode),
    body.category,
    body.language
  );
...
api.get('/level/:level', (c) => {
  ...
  return c.json<LevelLookupResponse>({
    type: 'level',
    tier: {
      level: tier.level,
      label: tier.label,
...
api.get('/profile', async (c) => {
  ...
  const bestPostUpvotes = await (async () => {
    try {
      ...
      const postIds: string[] = cipherListRaw ? (JSON.parse(cipherListRaw) as string[]) : [];
      const scores = await Promise.all(
        postIds.slice(0, 5).map(async (id) => {
          try {
            const p = await reddit.getPostById(id.startsWith('t3_') ? (id as `t3_${string}`) : `t3_${id}`);
            return p.score;
          } catch {
            return 0;
          }
...
api.get('/trending', async (c) => {
  ...
  const posts = (
    await Promise.all(
      topIds.map(async ({ member: id }) => {
        const cipher = await loadCipher(id);
        if (!cipher) return null;
        const submitterProfileRaw = await redis.get(keys.user(cipher.submitterUserId));
        const submitterProfile = submitterProfileRaw ? JSON.parse(submitterProfileRaw) : null;
        const tier = computeLevel(submitterProfile?.xp ?? 0);
        if (tier.level < TRENDING_MIN_LEVEL) return null;
        return {
  ...
  return c.json<TrendingResponse>({ type: 'trending', posts });
});

api.get('/leaderboard', async (c) => {
  const { userId } = context;
  const window = (c.req.query('window') ?? 'alltime') as 'weekly' | 'alltime';
  const board = (c.req.query('board') ?? 'decoders') as LeaderboardResponse['board'];
  const key =
    board === 'cipherMasters'
      ? window === 'weekly'
