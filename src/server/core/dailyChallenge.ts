...
type CipherOfDay = { postId: string; date: string };

const readStored = async (): Promise<CipherOfDay | null> => {
  const raw = await redis.get(keys.cipherOfDay());
  return raw ? (JSON.parse(raw) as CipherOfDay) : null;
};
...
export const getCipherOfDayPostId = async (): Promise<string | null> => {
...
export const pickCipherOfDay = async (): Promise<string | null> => {
  const today = todayUtc();
  const stored = await readStored();
  if (stored && stored.date === today) return stored.postId;
