...
import { keys, defaultUserProfile, currentIsoWeek, type StoredCipherPost } from './storage';
import { XP_REWARDS } from './leveling';
import { awardCipherMasterBadges } from './badges';

export const syncCipherUpvotes = async (cipher: StoredCipherPost): Promise<StoredCipherPost> => {
  try {
    const post = await reddit.getPostById(
      cipher.postId.startsWith('t3_') ? (cipher.postId as `t3_${string}`) : `t3_${cipher.postId}`
    );
    ...
    const targetTotal = Math.min(
      Math.max(0, liveScore) * XP_REWARDS.PER_UPVOTE,
      XP_REWARDS.MAX_UPVOTE_XP_PER_POST
    );
