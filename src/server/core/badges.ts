...
// `tier` powers the circular chip's accent color in the UI (Section 12:
// "Badges: circular icon chips, single accent color per tier
// (bronze/silver/gold)") — one-off achievements (first crack, first
// submission) have no tier and render in the default brand color instead.
export type BadgeDef = { id: string; icon: string; label: string; tier?: 1 | 2 | 3 };
...
export const awardMilestoneBadges = (profile: {
  ...
  for (const n of STREAK_MILESTONES) {
    if (profile.currentStreak >= n) badges.add(`streak-${n}`);
  }
  for (const n of DECODE_MILESTONES) {
    if (profile.totalDecodes >= n) badges.add(`decode-${n}`);
  }
...
export const awardCipherMasterBadges = (profile: {
  ...
  for (const n of CIPHER_MASTER_MILESTONES) {
    if (profile.cipherMasterScore >= n) badges.add(`cipher-master-${n}`);
  }
  return Array.from(badges);
};

export const toBadgeChips = (badgeIds: string[]): BadgeDef[] =>
  badgeIds.map((id) => BADGE_DEFS[id]).filter((def): def is BadgeDef => Boolean(def));
