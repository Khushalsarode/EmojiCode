...
import { keys, defaultUserProfile, applyStreak, answersFor, currentIsoWeek, } from './storage';
...
export const processGuess = async (input) => {
    ...
    if (cipher.statsCommentId) {
        try {
            ...
            const statsComment = await reddit.getCommentById(cipher.statsCommentId.startsWith('t1_') ? cipher.statsCommentId : `t1_${cipher.statsCommentId}`);
