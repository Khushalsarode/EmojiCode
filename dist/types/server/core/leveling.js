...
const dynamicTierForLevel = (level) => {
...
export const computeLevel = (xp) => {
    for (const tier of LEVEL_TIERS.slice(0, -1)) {
        if (xp >= tier.xpStart && xp <= tier.xpEnd)
            return tier;
    }
    if (xp < BASE_TIER.xpStart)
        return LEVEL_TIERS[0]; // defensive fallback, shouldn't hit
...
export const baseDailySubmissionLimit = (level) => {
...
export const flairColorForLevel = (level) => {
