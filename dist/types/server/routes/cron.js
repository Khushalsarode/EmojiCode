...
cron.post('/leaderboard-digest', async (c) => {
    ...
    if (topPlayers.length === 0 && topPosts.length === 0) {
        return c.json({}); // nothing to report yet — skip quietly
    }
    const playerLines = await Promise.all(topPlayers.map(async (entry, i) => {
    ...
    const postLines = await Promise.all(topPosts.map(async ({ member: postId, score }, i) => {
