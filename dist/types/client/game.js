...
const EMOJI_CHOICES = ['🎬', '🦁', '👑', '🌅', '🎶', '🐶', '🚀', '🏰', '🔥', '🌊', '🎮', '📺'];
const SubmitCipherModal = ({ onClose }) => {
...
export const App = () => {
    ...
    return (_jsxs("div", { className: "flex flex-col min-h-screen bg-white dark:bg-gray-900 px-4 py-6 gap-5", children: [_jsxs("div", { className: "flex items-center justify-between", children: [_jsx("button", { className: "text-xl text-gray-500 dark:text-gray-400", onClick: () => setScreen('menu'), "aria-label": "Menu", children: "\u2630" }), isSolved && (_jsx("button", { className: "text-xs text-gray-500 dark:text-gray-400 underline", onClick: () => setScreen('recap'), children: "\uD83D\uDCCA Recap" }))] }), post && (_jsxs(_Fragment, { children: [_jsx("div", { className: "text-6xl text-center tracking-widest", children: post.emojis.join(' ') }), _jsxs("div", { className: "text-center text-sm text-gray-500 dark:text-gray-400", children: [post.decoderCount, " players cracked it", post.firstCrackUsername && _jsxs(_Fragment, { children: [" \u00B7 \uD83E\uDD47 u/", post.firstCrackUsername] })] }), revealedAnswer ? (_jsxs("div", { className: "text-center text-base font-heading font-semibold text-gray-800 dark:text-gray-100", children: ["The answer was: ", revealedAnswer] })) : (_jsxs("div", { className: "flex flex-col gap-2 items-center", children: [_jsxs("div", { className: "flex w-full max-w-sm gap-2", children: [_jsx("input", { className: "flex-1 h-11 rounded-lg border border-gray-300 dark:border-gray-600 px-3 bg-transparent text-gray-900 dark:text-gray-100", placeholder: "My guess...", value: guessText, onChange: (e) => setGuessText(e.target.value), onKeyDown: (e) => e.key === 'Enter' && handleGuess(), disabled: submitting }), _jsx("button", { className: "h-11 px-4 rounded-lg text-white font-medium", style: { backgroundColor: 'var(--color-primary)' }, onClick: handleGuess, disabled: submitting, children: "Send" })] }), feedback && (_jsx("div", { className: "text-sm font-medium", style: {
                                    color: feedback.matched
                                        ? 'var(--color-success)'
                                        : feedback.closeMatch
                                            ? 'var(--color-warning)'
                                            : undefined,
                                }, children: feedback.message })), _jsx("button", { className: "text-xs text-gray-400 dark:text-gray-500 underline mt-1", onClick: handleGiveUp, children: "\uD83C\uDFF3 Give up" })] }))] })), _jsxs("div", { className: "mt-auto flex flex-col gap-2 w-full max-w-sm mx-auto", children: [_jsx("button", { className: "w-full h-11 rounded-lg border border-gray-300 dark:border-gray-600 font-medium text-gray-800 dark:text-gray-100", onClick: openCreateCipher, children: "\u2728 Create a Cipher" }), _jsx("button", { className: "w-full h-9 text-xs text-gray-400 dark:text-gray-500 underline", onClick: () => navigateTo('https://www.reddit.com/r/Devvit'), children: "r/Devvit" })] }), modalOpen && (_jsx(SubmitCipherModal, { onClose: () => {
                    setModalOpen(false);
                    void refreshProfile();
                } })), screen === 'menu' && (_jsx(HomeMenu, { profile: profile, onClose: () => setScreen(null), onCreateCipher: openCreateCipher, onOpenRewards: () => setScreen('rewards'), onOpenLeaderboard: () => setScreen('leaderboard'), onOpenLevelUp: () => setScreen('levelup') })), screen === 'rewards' && _jsx(MyRewards, { profile: profile, onClose: () => setScreen('menu') }), screen === 'levelup' && profile && _jsx(LevelUp, { profile: profile, onClose: () => setScreen('menu') }), screen === 'leaderboard' && _jsx(Leaderboard, { onClose: () => setScreen('menu') }), screen === 'recap' && _jsx(SolvedRecap, { onClose: () => setScreen(null), onCreateCipher: openCreateCipher })] }));
};
